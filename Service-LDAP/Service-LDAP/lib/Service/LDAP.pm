...
sub list_departments
{
	...
	croak("Service::LDAP::list_departments Failed. No Company supplied for search") unless $company;
...
sub user_create
{
	...
	my $sam = "$fname$lname";
	my $up = $sam."@"."Affinity.Test";
	my $manager = $param->{manager};
	my $dn_manager = "CN=$manager,OU=$dep,OU=$comp,$base";
	my $charmap = Unicode::Map8->new('latin1')  or  die;
	my $pass = mkpasswd(-length => 12, -minnum => 4, -minlower => 4, -minupper => 4, -minspecial => 0);
	...
	croak("Not a valid South African ID Number") unless $zaid;
...
sub forgot_pass
{
	...
	croak("Service::LDAP::forgot_pass Failed. No Email Address supplied") unless $mail;
	$mail =  "mail=".$param->{mail};

	my $search = $self->{ldap}->search( base => $base, filter => $mail, scope => "subree");
	my $Data = &_cn($self,$base,$mail);
	my $dn;
	my $cell;
	my $random = int( rand(1000)) + 9999;
	if($search->error  ne "Success"){
                croak "LDAP Bind  Failed. Could not bind to service. ".$search->error;
	}else{
		foreach my $key(keys %{$Data}){
			...
			my $otp = $param->{otp};
			croak("Service::LDAP::forgot_pass Failed. No Email OTP supplied") unless $otp;
				my $newpass = $param->{newpass};
                               	my $confirmpass = $param->{confirmpass};
				if($otp == $compare){
               				my $newpass = $param->{newpass};
					croak("Service::LDAP::forgot_pass Failed. No password supplied") unless $newpass;
        				my $confirmpass = $param->{confirmpass};
					croak("Service::LDAP::forgot_pass Failed. Please supply password again") unless $confirmpass;
					if($confirmpass eq $newpass){
					my $msg = $self->{ldap}->modify($dn, replace => {'userPassword' => $newpass } );
                       				if($msg->error  ne "Success"){
                         		      		croak ("Service::LDAP::forgot_pass Failed. ".$msg->error);
                       				}else{
                         			      	croak("Service::LDAP::forgot_pass. Password updated successfully!");
						}
					}else{
						croak("Service::LDAP::forgot_pass failed. Passwords do not match");
					}
               				
			}else{
				croak("Service::LDAP::forgot_pass Failed. ERROR\n");
			}
		}
	}
